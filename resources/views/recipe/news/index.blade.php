...
@section('nav')
    <div class="container-fluid">
        <a style="margin-right:1%" href="{{route('news.create')}}" class="btn btn-primary">+ Create</a>
        <div class="search">
            <form action="{{ route('new.search') }}" method="GET" class="d-flex" role="search">
        ...
        <table class="table table-success table-striped-columns ">
            ...
            @foreach($news as $lists)
                <tr>
                    ...
                    <td>{{$lists->title}}</td>
                    <td>{{$lists->content}}</td>
                    <td>{{$lists->time_day}}</td>
                    @can('edit')
                    <td>
                        <div class="action-container">
                            <a href="{{ route('news.edit', $lists->id) }}" class="edit">
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form action="{{ route('news.destroy', $lists->id)}}" method="POST">
                                                @csrf
                                                @method('DELETE')
